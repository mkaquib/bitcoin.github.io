    ...
    <Container maxW={'container.xl'}>
    {
        loading ? <Loader/> : <>
            ...
            <VStack spacing={'4'} p='16' alignItems={'flex-start'}>
                ...
                <CustmBar high={`${currencySymbol}${coin.market_data.high_24h[currency]}`} low={`${currencySymbol}${coin.market_data.low_24h[currency]}`}/>
                <Box w='full' p="4">
...
const Item = ({title,value})=>(
    <HStack justifyContent={['space-between','center']} w='full' my='4'>
        <Text fontFamily={'Bebas Neue'} letterSpacing='widest'>{title}</Text>
        <Text>{value}</Text>
    </HStack>
)

const CustmBar =({high,low})=>(
    <VStack w='full'>
